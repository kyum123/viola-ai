...
export default function RiskMap({ items, center }) {
  ...
  // WMS 오버레이 img 요소를 카카오맵 컨테이너 내부에 생성
  // z-index 150: 타일 레이어(~0) 위, 마커(~200+) 아래
  useEffect(() => {
    if (!ready || !containerRef.current) return;
  ...
  // WMS URL 갱신: 지도 이동·줌 시마다 현재 뷰포트에 맞는 이미지 요청
  useEffect(() => {
  ...
  return (
    <div className="relative h-full min-h-[400px] overflow-hidden rounded-xl">
      ...
      {ready && (
        <button
          onClick={() => setWmsVisible((v) => !v)}
          className={`absolute right-4 top-4 z-10 rounded-lg px-3 py-1.5 text-xs font-medium shadow transition ${
            wmsVisible
              ? "bg-blue-600 text-white"
              : "bg-white text-gray-600 border border-gray-200"
          }`}
        >
          침수흔적도 {wmsVisible ? "ON" : "OFF"}
        </button>
      )}
